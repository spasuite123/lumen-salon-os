...
/** Pass a 'YYYY-MM-DD' to scope to one calendar day (local time). Omit to get everything. */
export async function getAppointments(dateISO?: string) {
...
export async function createAppointment(a: {
  ...
  h: number; m: number; priceCents: number; durMin: number; dateISO?: string
}) {
  if (!isSupabaseConfigured) {
  ...
  const org = await getOrgId()
  const start = a.dateISO ? new Date(a.dateISO + 'T00:00:00') : new Date()
